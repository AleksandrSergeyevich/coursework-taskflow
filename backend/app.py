...
class TaskAdminModelView(ModelView):
    ...
    column_list = ['id', 'title', 'description', 'status', 'due_date', 'user_id', 'github_issue_number', 'created_at', 'completed_at']
    form_columns = ['title', 'description', 'status', 'due_date', 'user_id', 'github_issue_number', 'created_at', 'completed_at']
...
admin.add_view(TaskAdminModelView(Task, db.session, name='Задачи'))
admin.add_view(DashboardView(name='📊 Дашборд', endpoint='dashboard'))
...
# Эндпоинт для выхода из админки
@app.route('/admin/logout')
def admin_logout():
    session.pop('is_admin', None)
    return redirect('/admin/login')

# Кастомный индекс админки с кнопкой "Выйти"
...
admin.index_view = MyAdminIndexView(name='Главная', endpoint='admin')

# Создаём шаблон с кнопкой "Выйти"
os.makedirs('templates/admin', exist_ok=True)
with open('templates/admin/index.html', 'w') as f:
    f.write('''{% extends 'admin/master.html' %}
{% block body %}
{{ super() }}
<a href="/admin/logout" style="position: absolute; top: 10px; right: 10px; padding: 5px 10px; background: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer;">🚪 Выйти</a>
{% endblock %}''')
...
def initialize_once():
    ...
    if not _initialized:
        ...
        if not User.query.first():
            ...
            test_user.xp = 999
            test_user.level = 9
            test_user.badges = ["Администратор"]
...
# Геймификация: начисление XP и бейджей
def award_xp_and_badges(user, xp_amount):
    user.xp += xp_amount
    new_level = user.xp // 100
    if new_level > user.level:
        user.level = new_level
        logger.info(f"🏆 {user.username} достиг уровня {user.level}!")

    completed_count = Task.query.filter_by(user_id=user.id, status='Завершена').count()
    badges = set(user.badges or [])

    if completed_count >= 5 and "5 задач" not in badges:
        badges.add("5 задач")
        logger.info(f"🎖️ {user.username} получил бейдж '5 задач'!")
    if completed_count >= 10 and "Мастер продуктивности" not in badges:
        badges.add("Мастер продуктивности")
        logger.info(f"🎖️ {user.username} получил бейдж 'Мастер продуктивности'!")
...
def auth_telegram():
    ...
    data_check_string = "".join([f"{k}={v}" for k, v in sorted_data])
...
def create_task(current_user):
    try:
        ...
        # Начисление XP за создание задачи
        award_xp_and_badges(current_user, 5)
...
def update_task_status(current_user, task_id):
    try:
        ...
        if task.status == 'Завершена' and old_status != 'Завершена':
            task.completed_at = datetime.datetime.utcnow()
            award_xp_and_badges(current_user, 10)

        db.session.commit()

        if current_user.telegram_chat_id:
            message = f"✅ Задача обновлена!\n📌 {task.title}\n🔄 Статус: {old_status} → {task.status}"
            send_telegram_notification(current_user.telegram_chat_id, message)
...
def get_user_stats(current_user):
    try:
        ...
        completed = [t for t in tasks if t.status == 'Завершена']
        total = len(tasks)
